...
from config.settings import GIRO_BUFFER_SECONDS, SIMULATION_MODE
...
def _giro_long(symbol: str, qty: float, payload: dict) -> dict:
    ...
    log_trade(signal="GIRO_LONG_CLOSE", symbol=symbol, qty=qty_close,
              price=payload.get("price","0"), result=result_close,
              robot=payload.get("robot","CAZADOR"), demo=SIMULATION_MODE)
    log_trade(signal="GIRO_LONG_OPEN",  symbol=symbol, qty=qty_open,
              price=payload.get("price","0"), result=result_open,
              robot=payload.get("robot","CAZADOR"), demo=SIMULATION_MODE)
...
def _giro_short(symbol: str, qty: float, payload: dict) -> dict:
    ...
    log_trade(signal="GIRO_SHORT_CLOSE", symbol=symbol, qty=qty_close,
              price=payload.get("price","0"), result=result_close,
              robot=payload.get("robot","CAZADOR"), demo=SIMULATION_MODE)
    log_trade(signal="GIRO_SHORT_OPEN",  symbol=symbol, qty=qty_open,
              price=payload.get("price","0"), result=result_open,
              robot=payload.get("robot","CAZADOR"), demo=SIMULATION_MODE)
...
def _sl_long(symbol: str, signal: str, payload: dict = None) -> dict:
    ...
    result = close_all_positions(symbol, "LONG")
    log_trade(signal=signal, symbol=symbol, qty=0,
              price=payload.get("price","0") if payload else "0",
              result=result, robot=payload.get("robot","CAZADOR") if payload else "CAZADOR",
              demo=SIMULATION_MODE)
